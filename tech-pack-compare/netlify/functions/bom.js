/**
 * Path: netlify/functions/bom.js
 * Purpose: 單跑 bomAgent（model_tier: high → sonar-pro temp 0）。
 * Depends on:
 *   - netlify/functions/_lib/workflowService.js
 *   - src/agents/extractorAgent.js
 *   - src/agents/bomAgent.js
 *
 * 變更日期 / 改動原因：
 *   2026-05-19  初版（Phase 3）— BOM 升級對應 endpoint
 */
...
export async function handler(event, context) {
  return handleRequest(
    ...
    async ({ body, requestId, log }) => {
      ...
      const result = await runBomAgent({ input: extracted, requestId });
      warnings.push(...(result.warnings || []));
      if (!result.ok) {
        return errorResponse('BOM_FAILED', result.error || 'BOM 比對失敗', 500, null, { warnings });
      }
      log.info('bom.done', { count: result.data.length });
      return okResponse(
