/**
 * Path: netlify/functions/comments.js
 * Purpose: 單跑 commentAgent（model_tier: medium → sonar-pro）。
 * Depends on:
 *   - netlify/functions/_lib/workflowService.js
 *   - src/agents/extractorAgent.js
 *   - src/agents/commentAgent.js
 *
 * 變更日期 / 改動原因：
...
export async function handler(event, context) {
  return handleRequest(
    ...
    async ({ body, requestId, log }) => {
      ...
      const result = await runCommentAgent({ input: extracted, requestId });
      warnings.push(...(result.warnings || []));
      if (!result.ok) {
        return errorResponse('COMMENT_FAILED', result.error || '註解抽取失敗', 500, null, { warnings });
      }
      log.info('comments.done', { count: result.data.length });
      return okResponse(
